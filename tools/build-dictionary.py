...
Правила отбора:
  ...
  - pymorphy3 разбирает его как NOUN, sing, nomn, без пометок имён, фамилий, географии,
    аббревиатур. Вес разбора не проверяется: у неодушевлённых мужского и среднего
    рода именительный делит вероятность с винительным, и прежний порог 0,4 выбрасывал
    ЛЕС, ХЛЕБ, МОРЕ, ПОЛЕ — игрок находил такое слово и получал отказ «слова нет»
...
def main():
    ...
    for word in nouns:
        ...
        for parse in morph.parse(word):
            ...
            # Вес разбора не проверяем, и это принципиально. У неодушевлённых слов
            # мужского и среднего рода именительный и винительный совпадают, поэтому
            # pymorphy делит вероятность между ними: у ЛЕСА именительный получает 0,38,
            # у ХЛЕБА 0,35, у МОРЯ 0,22. Порог 0,4 выбрасывал целый класс бытовых слов —
            # игрок находил ЛЕС и получал красный отказ «такого слова нет».
            # Дополнительной проверки не нужно: список-источник и без того состоит
            # из существительных в именительном падеже, а вульгарное вырезает BLOCKLIST.
            if (
                tag.POS == "NOUN"
                and "sing" in tag
                and "nomn" in tag
                and not (grammemes & BAD_TAGS)
            ):
